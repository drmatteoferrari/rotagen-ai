...
export default function SurveyStep2() {
  ...
  const prevPath = isAdminMode ? `/admin/survey-override/${doctorId}/1` : "/doctor/survey/1";
  const nextPath = isAdminMode ? `/admin/survey-override/${doctorId}/3` : "/doctor/survey/3";
      ...
      <div className="flex flex-col min-h-full">
        ...
        <div className="absolute bottom-0 w-full bg-white/95 backdrop-blur-md border-t border-slate-100 p-4 pb-6 z-30">
          <div className="flex items-center justify-between gap-4">
            <button onClick={() => navigate(prevPath)} className="flex-1 rounded-xl bg-slate-100 py-3.5 text-center text-sm font-bold text-slate-700 hover:bg-slate-200 transition-colors">Back</button>
            <button onClick={() => navigate(nextPath)} className="flex-1 rounded-xl bg-teal-500 py-3.5 text-center text-sm font-bold text-white shadow-lg shadow-teal-500/25 hover:bg-teal-600 active:scale-[0.98] transition-all">Next Step</button>
          </div>
        </div>
      </div>
    </DoctorLayout>
