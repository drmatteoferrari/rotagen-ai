...
import { ArrowLeft, FileText, Heart, Activity, Ambulance, CheckCircle } from "lucide-react";

const competencies = [
  {
    code: "IAC", name: "Initial Assessment", icon: FileText,
    achieved: true, toggleLabel: "Independent Practice?", toggleSub: "Can you practice solo?", defaultChecked: true,
  },
  {
    code: "IAOC", name: "Obstetric Competence", icon: Heart,
    achieved: false, toggleLabel: "Working towards?", toggleSub: "Is this a current goal?", defaultChecked: false,
    yesNoDefault: "no",
  },
  {
    code: "ICU", name: "Intensive Care Unit", icon: Activity,
    achieved: true, toggleLabel: "Independent Practice?", toggleSub: "Cleared for solo shifts?", defaultChecked: true,
    yesNoDefault: "yes",
  },
  {
    code: "Transfers", name: "Patient Transport", icon: Ambulance,
    achieved: false, toggleLabel: "Working towards?", toggleSub: "Actively training?", defaultChecked: true,
    yesNoDefault: "no",
  },
];
    ...
    <DoctorLayout>
      <div className="flex flex-col min-h-full">
        <header className="sticky top-0 z-20 flex items-center justify-between bg-[#f6f8f8]/95 backdrop-blur-sm p-4 pb-2">
          <button onClick={() => navigate("/doctor/survey/1")} className="flex size-10 items-center justify-center rounded-full text-slate-900 hover:bg-slate-200 transition-colors">
        ...
        <div className="absolute bottom-0 w-full bg-white/95 backdrop-blur-md border-t border-slate-100 p-4 pb-6 z-30">
          <div className="flex items-center justify-between gap-4">
            <button onClick={() => navigate("/doctor/survey/1")} className="flex-1 rounded-xl bg-slate-100 py-3.5 text-center text-sm font-bold text-slate-700 hover:bg-slate-200 transition-colors">
              Back
            </button>
            <button onClick={() => navigate("/doctor/survey/3")} className="flex-1 rounded-xl bg-teal-500 py-3.5 text-center text-sm font-bold text-white shadow-lg shadow-teal-500/25 hover:bg-teal-600 active:scale-[0.98] transition-all">
              Next Step
            </button>
          </div>
        </div>
