      ...
      <div className="flex flex-col min-h-full">
        <header className="sticky top-0 z-10 bg-[#f6f8f8]/95 backdrop-blur-sm border-b border-slate-200">
          <div className="flex items-center p-4 justify-between h-16">
            <button onClick={() => navigate("/doctor/survey/4")} className="flex items-center justify-center w-10 h-10 rounded-full hover:bg-slate-200 transition-colors text-slate-900">
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-4 pb-8 z-20">
          <div className="flex gap-4">
            <button onClick={() => navigate("/doctor/survey/4")} className="flex-1 py-3.5 rounded-xl border border-slate-200 text-slate-700 font-bold hover:bg-slate-50 transition-colors">Back</button>
            <button onClick={() => navigate("/doctor/survey/6")} className="flex-1 py-3.5 rounded-xl bg-teal-500 text-white font-bold shadow-lg shadow-teal-500/30 hover:bg-teal-600 transition-colors flex items-center justify-center gap-2">
