# RotaGen Final Rota Algorithm — Complete Specification v2.9
...
> **Supersedes:** v2.8. This version reconciles the specification with the live repository and Lovable deployment after Prompts 1 and 2. No algorithm **rules** change between v2.8 and v2.9 — every rule (A0–A21, B22–B30, C31–C32, D33–D41, E42–E48, F49–F54, G55–G61, H62–H65, I66–I72) is retained unchanged. The changes are structural: the public output contract has been simplified to match the UI's actual needs, the DB schema has been rewritten to match the deployed migration, the Web Worker runtime has been documented, the `cancelled` status path has been added, and a clean separation has been introduced between the algorithm's internal working types (§4.1 — rich) and the public boundary types (§4.2 — simplified, what the UI and DB see). All v2.8 behaviour is preserved.
...
**Clarification (v2.8):** This is an algorithm responsibility, not a `buildFinalRotaInput` responsibility. Phase 0 of the algorithm reads `resolved_availability` directly to build the Availability Matrix (§5.1). `buildFinalRotaInput` supplies `constraints.hard` leave arrays as a secondary pre-screen hint; the authoritative leave computation (§5.2) runs inside the algorithm from `resolved_availability`. Both sources should be consistent. `buildFinalRotaInput` now reads leave from `unavailability_blocks` (canonical relational source) rather than JSONB, reducing staleness risk.
...
**doctors[]:** Full type shown in §4. Key contract rules:
...
- `ltft.daysOff` = from `ltft_patterns` table (canonical), JSONB fallback [v2.8 — F5]
...
- `constraints.hard.rotationDates` expanded from `unavailability_blocks` reason='rotation' [Bug 4, v2.8 — F5]
- `constraints.hard.annualLeaveDates/studyLeaveDates/parentalLeaveDates/nocDates` from `unavailability_blocks` (canonical), JSONB fallback [v2.8 — F5]
- `trainingPreferences` and `dualSpecialty` included as informational fields — not enforced in v1 [Bug 6]

**constraints:** named hard[] and soft[] identifiers
...
```typescript
// ── Input: per-doctor in FinalRotaInput ───────────────────────

interface FinalRotaDoctor {
  doctorId: string;
  name: string;
  grade: string;
  wtePct: number;
  contractedHoursPerWeek: number;   // from targets_data.contractedHoursPerWeek [v2.8 — F5]
  hasIac: boolean;                  // from iac_achieved flat column, JSONB fallback [Bug 1]
  hasIaoc: boolean;
  hasIcu: boolean;
  hasTransfer: boolean;
  ltft: {
    isLtft: boolean;                // daysOff.length > 0, NOT wtePct < 100 [Bug 9]
    daysOff: string[];              // from ltft_patterns, lowercase day names e.g. ["friday"]
    nightFlexibility: Array<{
      day: string;
  ...
  constraints: {
    hard: {
      annualLeaveDates: string[];   // from unavailability_blocks reason='annual'
      studyLeaveDates: string[];    // from unavailability_blocks reason='study'
      parentalLeaveDates: string[]; // from unavailability_blocks reason='parental'
      rotationDates: string[];      // from unavailability_blocks reason='rotation' [Bug 4]
    ...
    soft: {
      nocDates: string[];           // from unavailability_blocks reason='noc'
      maxConsecNights: number;
      additionalNotes: string;
    };
  };
  fairnessTargets: {
    targetTotalHours: number;       // targets_data.totalMaxHours — post-leave [v2.8 — F5]
    targetNightShiftCount: number;  // estimated from shiftSlots demand
    targetWeekendShiftCount: number;
    targetOncallCount: number;
    proportionFactor: number;       // wtePct / 100
  };
  shiftTargets: DoctorShiftTarget[]; // from pre_rota_results.targets_data [v2.8 — F5]
  totalMaxHours: number;             // targets_data.totalMaxHours
  weekendCap: number;                // targets_data.weekendCap
  hardWeeklyCap: number;             // targets_data.hardWeeklyCap (= wtrMaxHoursPer168h)
  trainingPreferences: Array<{       // [Bug 6] Informational only — not enforced in v1
    category: 'specialty' | 'session' | 'interest';
    name: string;
    notes: string | null;
  }>;
...
**CRITICAL:** The Availability Matrix MUST be built from `resolved_availability` rows, NOT from raw survey JSONB or `unavailability_blocks` directly.

For each doctor × date combination, read the `status` value from `resolved_availability` (UNIQUE on rota_config_id, doctor_id, date). Map to `canWork`:
...
- **LTFT edge flexibility:** If status is `LTFT` (day off), check `can_start_nights` and `can_end_nights` for night block boundary positions only. All other leave types produce a hard `false` with zero flexibility.
...
| **Doctor availability (canonical)** | **`resolved_availability`** | **Availability Matrix (Phase 0, direct read)** | **[Bug 5] ONLY source — incorporates coordinator overrides. Never use survey JSONB for availability.** |
...
| Annual leave dates | `unavailability_blocks` reason='annual' | `doctors[].constraints.hard.annualLeaveDates` | [v2.8 — F5] Relational tables first, JSONB fallback. Expanded ISO dates. |
| Study leave dates | `unavailability_blocks` reason='study' | `doctors[].constraints.hard.studyLeaveDates` | [v2.8 — F5] |
| Parental leave dates | `unavailability_blocks` reason='parental' | `doctors[].constraints.hard.parentalLeaveDates` | [v2.8 — F5] |
| **Rotation dates** | **`unavailability_blocks` reason='rotation'** | **`doctors[].constraints.hard.rotationDates`** | **[Bug 4, v2.8 — F5] Was missing — now populated** |
| NOC dates | `unavailability_blocks` reason='noc' | `doctors[].constraints.soft.nocDates` | [v2.8 — F5] Relational tables first, JSONB fallback |
| LTFT patterns | `ltft_patterns` | `doctors[].ltft.daysOff` + `nightFlexibility[]` | [v2.8 — F5] `isLtft = daysOff.length > 0` [Bug 9]. Relational tables first, JSONB fallback. |
